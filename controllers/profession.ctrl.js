...
const Profession_controller = {
  ...
  create_profession: (req, res) => {
    try {
      ...
      newProfession.save()
      res.status(201).json({ message: 'Profession created successfully', profession: newProfession });
    } catch (error) {
      console.error(error);
  ...
  assign_courses: async (req, res) => {
    try {
      ...
      if (!professionId || !courseIdsWithOrder || !Array.isArray(courseIdsWithOrder)) {
      ...
      res.status(200).json({ message: 'Courses assigned successfully', profession });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Server error', error });
    }
  ...
  enroll_in_profession: async (req, res) => {
    try {
      ...
      if (user.enrolledProfessions.includes(professionId)) {
        return res.status(400).json({ message: 'User already enrolled in this profession' });
      }

      if (user.currentCourse && user.currentModule) {
        return res.status(400).json({ message: 'Cannot enroll in a new profession while enrolled in a course. Please complete or unenroll from the current course first.' });
      }
      ...
      await user.save();
      res.status(200).json({ message: 'Enrolled in profession successfully', profession });
  ...
  unenroll_from_profession: async (req, res) => {
    try {
      ...
      if (index === -1) {
        return res.status(400).json({ message: 'User is not enrolled in this profession' });
      }
      user.enrolledProfessions.splice(index, 1);
      if (user.currentProfession && user.currentProfession.toString() === professionId) {
        user.currentProfession = null;
      }
      // If the user's current course/module belongs to this profession, clear them
      if (user.currentCourse) {
        const isCurrentCourseInProfession = profession.courses.some(c => c.course.toString() === user.currentCourse.toString());
      ...
      await user.save();
      res.status(200).json({ message: 'Unenrolled from profession successfully', profession });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Server error', error });
    }
